...
def parse_args():
    ...
    parser.add_argument("-image_num", type=int, default=200)
...
def gen_query(image_path, exp_image_paths, exp_image_attr_lists, verbose=0):
    ...
    prompt = {"type":
              ...
              "text":
                  "-Role: 人脸图像特征标注任务。"
                  "-Background: 人脸图像需要标注的特征有：['Attractive', 'Big_Nose', 'Blond_Hair', 'Heavy_Makeup', 'Male', 'Smiling']。具体特征含义为：{Attractive：图像中人脸是否具有吸引力；Big_Nose：图像中人脸鼻子是否较大的鼻子；Blond_Hair：图像中人脸是否是金发；Heavy_Makeup：图像中人脸是否有明显化妆；Male图像中人脸的性别是否为男性；Smiling：图像中人脸是否在微笑。}"
