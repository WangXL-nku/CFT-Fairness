...
class Craft(BaseConceptExtractor):
    ...
    def transform(self, inputs: np.ndarray, activations: Optional[np.ndarray] = None):
        ...
        W_dtype = self.reducer.components_.dtype
        activations_np = torch_to_numpy(activations).astype(W_dtype)

        if np.all(activations_np == 0):
            activations_np = activations_np + np.random.normal(0, 1e-6, activations_np.shape)

        U = self.reducer.transform(activations_np)
