...
def gen_large_fig(img_list):
    ...
    large_image = Image.new('RGB', (8 * 10, 8 * 10), 'black')
    ...
    for i in range(len(img_list)):
        ...
        # 将小图片粘贴到大图片的相应位置
        large_image.paste(img_list[i], (x, y))

        # 如果不是最后一列，绘制右边的白线
        if col < 7:
            large_image.paste((255, 255, 255), (x + 10, y, x + 11, y + 11))

        # 如果不是最后一行，绘制下面的白线
        if row < 7:
            large_image.paste((255, 255, 255), (x, y + 10, x + 11, y + 11))
    return large_image
...
for model_name in model_name_list:
    ...
    for class_id in range(10):
        ...
        for i in range(10):
            image_save_dir = os.path.join(img_dir, str(model_name), str(class_id))
            if not os.path.exists(image_save_dir):
                os.makedirs(image_save_dir)
            concept_img_save_path = os.path.join(image_save_dir, f'Concept{i}.png')
            crop_list, image_list = load_crops(crops, crops_importances, images, i)
